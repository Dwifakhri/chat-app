<script setup>
import { ref, computed, onMounted, watch, onUnmounted, onBeforeMount, onBeforeUnmount } from 'vue';
  ...
  if (socket) {
    ...
    socket.value?.on("roomx", (arg) => {
      ...
      if (existChat) {
        ...
        detail.value.push({
          id: arg.room, name: arg.room, isGroup: true, member: [...arg.member], list: !arg.isInvite ? [arg.list] : []
        })
      }
    })
...
onBeforeUnmount(() => {
  detail.value.map((item) => {
    if (item.isGroup) {
      socket.value?.emit("leaveRoom", { room: item, name: me })
    }
  })
...
const createGroup = () => {
  ...
  if (!existChat) {
    detail.value.push({
      id: groupName.value, name: groupName.value, isGroup: true, member: [...selectedUsers.value, me], list: selectedUsers.value.map((item) => {
        return { name: item, time: new Date(), isInfo: true }
      }),
    })
